...
def foo():
    ...
    if sensorId == "84:f3:eb:44:d8:24":
        print("MLK/Central")
        producer.send(config['KAFKA']['central_topic'], request.get_data())
    elif sensorId == "84:f3:eb:91:44:60":
        print("MLK/Douglas")
        producer.send(config['KAFKA']['douglas_topic'], request.get_data())
    elif sensorId == "84:f3:eb:45:1a:53":
        print("MLK/Magnolia")
        producer.send(config['KAFKA']['magnolia_topic'], request.get_data())
    elif sensorId == "84:f3:eb:91:44:38":
        print("MLK/Peeples")
        producer.send(config['KAFKA']['peeples_topic'], request.get_data())
